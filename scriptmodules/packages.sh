...
function rp_createBin() {
    ...
    if [[ "$ret" -eq 0 ]]; then
        cp "$md_inst/emulos.pkg" "$dest/$md_id.pkg"
    else
        rm -f "$dest/$archive"
    fi
    return "$ret"
...
function rp_setPackageInfo() {
    ...
    local pkg="$install_path/emulos.pkg"
    local origin="$2"
...
function rp_loadPackageInfo() {
    ...
    local load=0
    local pkg_file="$(rp_getInstallPath $id)/emulos.pkg"
...
function rp_registerAllModules() {
    ...
    while read dir; do
        # get parent folder
        vendor="${dir%/*}"
        # if the folder isn't EmulOS-Setup then get the repo name which will be used for module vendor
