...
function main_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "$BACKTITLE" --title "MENÚ PRINCIPAL" \
            ...
            --menu "¿Qué lista de colección desea agregar/actualizar??" 25 75 20 \
            1 "Atari Classics" \
...
dialog --infobox "...creando lista..." 3 24 ; sleep 2
...
do
  ...
  if [[ $romfolder = "arcade" ]]; then
    ifexist=`ls /home/$user/EmulOS/roms/${romfolder} |grep -w "${gname}.zip"`
    if [[ -f "/home/$user/EmulOS/roms/${romfolder}/$ifexist" ]]; then
      echo "/home/$user/EmulOS/roms/${romfolder}/${ifexist}" >> /tmp/tempfile.cfg
    else
      :
    fi
    ...
    ifexist=`ls /home/$user/EmulOS/roms/${romfolder} |grep -w "${gname}"`
    if [[ -f "/home/$user/EmulOS/roms/${romfolder}/$ifexist" ]]; then
      echo "/home/$user/EmulOS/roms/${romfolder}/${ifexist}" >> /tmp/tempfile.cfg
     else
      :
...
done < /home/$user/EmulOS-Setup/scriptmodules/supplementary/extrasemulosmenu/escollections/${filename}
