...
function _update_hook_retropiemenu() {
    ...
    if ! rp_isInstalled "$md_id" && [[ -f "$home/.emulationstation/gamelists/emulos/gamelist.xml" ]]; then
        mkdir -p "$md_inst"
...
function configure_retropiemenu()
{
    ...
    local descs=(
        ...
        'Install, uninstall, or update EmulationStation themes. Most themes can be previewed at https://emulos.org.uk/docs/Themes/.'
    ...
    setESSystem "EmulOS" "emulos" "$rpdir" ".rp .sh" "sudo $scriptdir/emulos_packages.sh retropiemenu launch %ROM% </dev/tty >/dev/tty" "" "emulos"
