...
rp_module_desc="EmulOS-Setup Docs Viewer"
...
function gui_docsview() {
    local dir="$rootdir/EmulOS-Docs"
    while true; do
        local cmd=(dialog --backtitle "$__backtitle" --menu "EmulOS-Setup Docs Viewer" 22 76 16)
        local options=()
        if [[ -d "$dir" ]]; then
            options=(
                1 "Update EmulOS-Setup Docs"
                2 "View Pages"
                3 "Remove EmulOS-Setup Docs"
            )
        else
            options+=("1" "Download EmulOS-Setup Docs")
        fi
        local choice=$("${cmd[@]}" "${options[@]}" 2>&1 >/dev/tty)
        if [[ -n "$choice" ]]; then
            case "$choice" in
